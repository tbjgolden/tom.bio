...
const predictedFlightDate = new Date(2021, 8, 8, 12, 0, 0, 0)

export default function Experiments(): JSX.Element {
  const [daysUntilMove, setDaysUntilMove] = useState(differenceInDays(predictedFlightDate, new Date()))
  
  useEffect(() => {
    let timeout
    const updateDate = () => {
      const now = new Date()
      setDaysUntilMove(differenceInDays(predictedFlightDate, now))
      ...
      <div id="calendar">
        {
          daysUntilMove > 0 ?
            ...
            Moving to London
            <br />
            <small>(predicted flight date: {predictedFlightDate.toLocaleDateString()})</small>
            </>
            : <strong className="big">{
              daysUntilMove === 0
                ? "Moving Day! 🎉"
                : `Moved on ${predictedFlightDate.toLocaleDateString()}!`
            }</strong>
        }
      </div>
      <style jsx>{`
